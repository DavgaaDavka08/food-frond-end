...
export function FoodMenuDialogDemo() {
    ...
    const getData = async () => {
        try {
            const getCategory = await fetch("http://localhost:2000/api/food-category/:foodCategoryId")
            const jsonCategory = await getCategory.json()
            console.log('jsonCategory :>> ', jsonCategory);
            setGetCategory(jsonCategory.getfood
            )// getfood maani isfect iin getfoodshvv
        } catch (error) {
    ...
    const addData = async (categoryName: string) => {
        try {
            const addCategory = await fetch("http://localhost:2000/api/food-category", {
                method: "POST",
                headers: {
    ...
    const deleteData = async (id: string) => {
        try {
            const deleteCategory = await fetch(`http://localhost:2000/api/food-category/${id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                }
    ...
    const updateData = async (id: string, categoryName: string) => {
        try {
            const updatedata = await fetch(`http://localhost:2000/api/food-category/${id}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    return <Dialog open={open} onOpenChange={setOpen}>
        <div className="flex py-2 px-4 items-center gap-8 ">
            {getCategory?.map((data: FoodType, index) => {
                return (<ContextMenu key={index}>
                    <ContextMenuTrigger className="border-[1px] border-black px-5 rounded-full cursor-pointer py-2">
                        <p className="  text-[#18181B] text-sm font-medium leading-5">{data.categoryName}</p>
                    </ContextMenuTrigger>
                    <ContextMenuContent className="w-64">
                        ...
                        <Dialog >
                            ...
                            <DialogContent className="sm:max-w-[425px]">
                                ...
                                <DialogFooter>
                                    <Button onClick={() => updateData(data._id, updateDatas)} type="submit">
                                        Save changes
                                    </Button>
                                </DialogFooter>
                            </DialogContent>
            ...
            <DialogTrigger asChild>
                <Button variant="outline" className="flex w-[40px] h-[40px] py-2 px-4 items-center gap-2 rounded-full bg-[#EF4444]">
